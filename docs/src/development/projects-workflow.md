...
1. **Backlog**
   ...
   - Purpose: This status serves as a queue for upcoming work, allowing team members to easily identify tasks that are
     ready to start.

2. **In Progress**
   - Definition: Work that is actively being developed or implemented.
   - Purpose: This status indicates that a task is currently being worked on, providing visibility into ongoing activities
     within the team.

3. **Waiting**
   - Definition: Work that is temporarily paused, pending external dependencies.
   - Purpose: This status helps to highlight tasks that cannot proceed due to factors outside the team's control, ensuring
     that these items are monitored until they can be resumed.

4. **In Review**
   - Definition: Completed work that is under review, testing, or approval.
   - Purpose: This status signifies that a task has been finished but is awaiting feedback or validation from team members
     or stakeholders before it can be considered complete.
